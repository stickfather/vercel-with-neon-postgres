...
function mapRowToStudentBasicDetails(row: SqlRow, fallbackId: number): StudentBasicDetails {
  return {
    ...
    isUnderManagementSupervision: normalizeFieldValue(row.isUnderManagementSupervision, "boolean"),
...
export async function updateStudentBasicDetails(
  ...
  const query = `
    ...
    RETURNING
      ...
      COALESCE(s.is_under_management_supervision, false) AS "isUnderManagementSupervision",
...
function mapExamRow(row: SqlRow, fallbackStudentId: number): StudentExam {
  ...
  return {
    ...
    status: normalizeFieldValue(row.status, "text"),
    examType: normalizeFieldValue(row.exam_type, "text"),
...
export async function listStudentExams(studentId: number): Promise<StudentExam[]> {
  ...
  const rows = normalizeRows<SqlRow>(await sql`
    SELECT id, student_id, time_scheduled, status, exam_type, level, score, passed, notes
...
export async function createStudentExam(
  ...
  const rows = normalizeRows<SqlRow>(await sql`
    ...
    VALUES (
      ...
      ${data.examType ? data.examType.toLowerCase() : null},
      ${data.level},
      ${data.score},
      ${data.passed},
      ${data.notes}
    )
    RETURNING id, student_id, time_scheduled, status, exam_type, level, score, passed, notes
  `);
...
export async function updateStudentExam(
  ...
  const rows = normalizeRows<SqlRow>(await sql`
    ...
    SET time_scheduled = ${data.timeScheduled},
      status = ${data.status},
      exam_type = ${data.examType ? data.examType.toLowerCase() : null},
      level = ${data.level},
      score = ${data.score},
      passed = ${data.passed},
      notes = ${data.notes}
